"""
GDN decode entry (`config.toml`: msinfer_entry.py::run).

Default body is the official PyTorch reference from the track definition so
`pack_solution` / Modal runs work out of the box. Swap in your msinfer +
CUTLASS DSL implementation while keeping this `run(...)` signature.
"""
